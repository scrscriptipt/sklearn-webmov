...
df1['genres'] = df1['genres'].apply(literal_eval) #영화 장르 값이 문자열이라 딕셔너리로 변환
...
if col_title.button('영화 추천 받기'): #버튼 값이 참이 되었을 때
    with st.spinner('검색하는 중...'): #로딩 창 보여주기
        ...
        st.sidebar.write('')
        if (df2.loc[indx2[title],'age']) == '':
            st.sidebar.subheader('3.웹툰 연령대')
            st.sidebar.error('연령대 정보를 가지고 올 수 없습니다.')
            st.sidebar.write('')
        else:
            st.sidebar.subheader('3.웹툰 연령대')
